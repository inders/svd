  ...
  public void score(long user, @Nonnull MutableSparseVector scores) {
    ...
    for (VectorEntry e: scores.fast(VectorEntry.State.EITHER)) {
      ...
      if (userVector != null && itemVector != null) {
        double baseLineScore = baselineScorer.score(user, item);
        // R = U * FEATURE WEIGHT MATRIX * transpose(V)
        // U - user feature matrix
        // V - item feature matrix
        RealMatrix result = userVector.multiply(weights.multiply(itemVector.transpose()));
        double finalScore = baseLineScore + result.getEntry(0, 0);
        scores.set(item, finalScore);
      }
    }
  }
