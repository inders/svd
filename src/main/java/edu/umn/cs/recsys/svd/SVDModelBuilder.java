...
public class SVDModelBuilder implements Provider<SVDModel> {
  ...
  public SVDModel get() {
    ...
    // TODO Truncate the matrices to featureCount and construct the SVD model
  ...
  private RealMatrix createRatingMatrix(IdIndexMapping userMapping, IdIndexMapping itemMapping) {
    ...
    try {
      for (UserHistory<Event> user: users) {
        ...
        baselines.subtract(ratings);
        for (VectorEntry vectorEntry : baselines.fast()) {
          long itemId = vectorEntry.getKey();
          double normalizedRating = vectorEntry.getValue();


          if (ratings.get(itemId) > 0) { //if user has not rated the item leave it blank
            //set the actual rating
            matrix.setEntry(u, itemMapping.getIndex(itemId), normalizedRating);
          }
        }
      }
    } finally {
